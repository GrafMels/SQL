...
# VIEW - табличка которая не добавляется в базу данных но сохраняется в памяти

* CREATE VIEW - создать табличку(скопировать)

# Транзакции

* ACID

1. Atomicity(Атомарность) - свойство которое говорит что будут совершены все действия в цеппочке, или не будет совершенно не одно действие
...
2. Consistency(Согласованость) - исходные данные на входе и на выходе должны быть подобными(однородными)
...
3. Isolation(Изоляция) - свойство которое задаёт порядок выполнения операций(на счёт поступает 200 р и 300 р если данные про 200 р не успеют записаться тогда новая сумма не пойдёт во входные данные и 300 рублеё запишутся поверх 200 р в итогу общий прирост будет всего + 300 р)
...
4. Durabillity(Долговечность) - Независимо от проблем на нижних уровнях (к примеру, обесточивание системы или сбои в оборудовании) изменения, сделанные успешно завершённой транзакцией, должны остаться сохранёнными после возвращения системы в работу. Другими словами, если пользователь получил подтверждение от системы, что транзакция выполнена, он может быть уверен, что сделанные им изменения не будут отменены из-за какого-либо сбоя.
